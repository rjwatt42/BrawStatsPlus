...
drawPossible <- function(IV,DV,effect,design,possible,possibleResult){
  ...
  switch (possible$view,
          "3D"= {
            ...
            z[is.na(z)] <- zlim[1]
            par(bg=graphcolours$graphC,mar=c(0,1,0,0),font.lab=2)
...
# lines on the floor
              ...
              # populations 
              if (possible$type=="Samples" && !is.na(possible$targetSample)) {
                lines(trans3d(x=xlim,y=c(0,0)+possible$targetSample,z=c(0,0)+zlim[1],pmat=mapping),col=colVline,lwd=2)
              }
                ...
                if (possible$type=="Samples") {
                  ...
                  if (!isempty(sRho)){
                    for (si in 1:length(sRho)) {
                      ...
                      if (logZ) z<-log10(z)
                      lines(trans3d(x=c(0,0)+xlim[1],y=c(sRho[si],sRho[si]),z=c(zlim[1],z)*wallHeight,pmat=mapping),col=colVline,lwd=2)
                    }
                  }
                }
                if (possible$type=="Populations" &&!is.na(possible$targetSample)) {
                      ...
                      for (i in order(pRho)) {
                        ...
                        if (possible$possibleTheory){
                          ...
                          while (length(z_use)>0 && any(z_use>0)) {
                            ...
                            rs_use<-rs_draw[use]
                            polygon (trans3d(x = rep(pRho[i],length(rs_use)+2), y = c(rs_use[1],rs_use,rs_use[length(rs_use)]), z = c(zlim[1],z_draw[use],zlim[1]), pmat = mapping), col = col, lwd=1)
                          ...
                          if (possible$textResult || doTextResult) {
                            ...
                            if (possible$UsePrior!="none") {
                              text(trans3d(x=xlim[1],y=mean(ylim),z=zlim[2]*1.05,pmat=mapping),
                                   labels=bquote(llr(italic(.(param))["+"]/italic(.(param))[0])==bold(.(format(-llrNull,digits=3)))),
